...
static void parseExtraArgs(int argc, char **argv, HeteroOptions *options)
{
  for (int arg = 5; arg < argc; ++arg) {
    if (strcmp(argv[arg], "--cpu-workers") == 0) {
      if (++arg >= argc ||
          !parseNonNegativeInt(argv[arg], &options->cpu_workers)) {
        fprintf(stderr, "--cpu-workers requires a non-negative integer\n");
        exit(-1);
      }
    } else if (strcmp(argv[arg], "--gpu-cus") == 0) {
      if (++arg >= argc ||
          !parseNonNegativeInt(argv[arg], &options->gpu_cus) ||
          options->gpu_cus == 0) {
...
int main(int argc, char ** argv)
{
  if (argc < 5) {
    ...
    fprintf(stderr, "Options:\n"
            ...
            "\t--gpu-cus N      Expected GPU CU count.\n"
            "\t--debug-log      Print CPU worker lifecycle logs.\n");
    exit(-1);
  }
  ...
  if (options.gpu_cus > 0 && options.gpu_cus != NUM_CU) {
    fprintf(stderr, "--gpu-cus %d does not match HIP device CU count %d\n",
            options.gpu_cus, NUM_CU);
    exit(-1);
  }
