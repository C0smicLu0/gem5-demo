...
int main(int argc, char *argv[])
{
  ...
  int nextArg = 8;
  HaccExecutionOptions haccOptions = {0, 0, false, false, false};
  if (argc > 8 && strncmp("rcb", argv[8], 3) == 0) {
    useRCB = argv[8][3] == 'm' ? 2 : 1;
    nextArg = 9;
    if (nextArg < argc && argv[nextArg][0] != '-') {
      rcbN = atoi(argv[nextArg++]);
      if (nextArg < argc && argv[nextArg][0] != '-') {
        tMin = atoi(argv[nextArg++]);
        if (nextArg < argc && strcmp("r", argv[nextArg]) == 0) {
          rpart = 1;
          ++nextArg;
        }
      }
    ...
    bhAll = 1;
    nextArg = 9;
  } else if (argc > 8 && strcmp("bh", argv[8]) == 0) {
    nextArg = 9;
  }
  ...
  for (int arg = nextArg; arg < argc; ++arg) {
    if (strcmp(argv[arg], "--cpu-workers") == 0) {
      if (++arg >= argc ||
          !parseNonNegativeInt(argv[arg], &haccOptions.cpuWorkers)) {
        fprintf(stderr, "--cpu-workers requires a non-negative integer\n");
        exit(-1);
      }
    } else if (strcmp(argv[arg], "--gpu-cus") == 0) {
      if (++arg >= argc ||
          !parseNonNegativeInt(argv[arg], &haccOptions.gpuCus) ||
          haccOptions.gpuCus == 0) {
        fprintf(stderr, "--gpu-cus requires a positive integer\n");
        exit(-1);
      }
    } else if (strcmp(argv[arg], "--cpu-only") == 0) {
      haccOptions.cpuOnly = true;
    } else if (strcmp(argv[arg], "--gpu-only") == 0) {
      haccOptions.gpuOnly = true;
    } else if (strcmp(argv[arg], "--debug-log") == 0) {
      haccOptions.debugLog = true;
    } else {
      fprintf(stderr, "unknown option: %s\n", argv[arg]);
      exit(-1);
    }
  }
  if (haccOptions.cpuOnly && haccOptions.gpuOnly) {
    fprintf(stderr, "--cpu-only and --gpu-only cannot be combined\n");
    exit(-1);
  }
  if (haccOptions.cpuOnly && haccOptions.cpuWorkers == 0) {
    fprintf(stderr, "--cpu-only requires --cpu-workers N with N > 0\n");
    exit(-1);
  }
  if (!haccOptions.cpuOnly && !haccOptions.gpuOnly &&
      haccOptions.cpuWorkers > 0 && haccOptions.gpuCus == 0) {
    fprintf(stderr, "mixed HACC execution requires --gpu-cus N\n");
    exit(-1);
  }
  configureHaccExecution(haccOptions);
  ...
  POSVEL_T* m_xArr;
  hipMallocManaged(&m_xArr, Np*sizeof(POSVEL_T));
  POSVEL_T* m_yArr;
  hipMallocManaged(&m_yArr, Np*sizeof(POSVEL_T));
  POSVEL_T* m_zArr;
  hipMallocManaged(&m_zArr, Np*sizeof(POSVEL_T));
  POSVEL_T* m_vxArr;
  hipMallocManaged(&m_vxArr, Np*sizeof(POSVEL_T));
  POSVEL_T* m_vyArr;
  hipMallocManaged(&m_vyArr, Np*sizeof(POSVEL_T));
  POSVEL_T* m_vzArr;
  hipMallocManaged(&m_vzArr, Np*sizeof(POSVEL_T));
  POSVEL_T* m_massArr;
  hipMallocManaged(&m_massArr, Np*sizeof(POSVEL_T));
  for(int i = 0; i < Np; i++) {
        m_xArr[i] = 0;
        m_yArr[i] = 0;
        m_zArr[i] = 0;
