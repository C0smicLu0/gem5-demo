...
static void
cpu_ini_range(int *max_array, int *min_array, VertexRange range)
{
    for (int tid = range.begin; tid < range.end; ++tid) {
        max_array[tid] = -1;
        min_array[tid] = BIG_NUM;
    }
}

static void
cpu_color1_range(const csr_array *csr, int *node_value, int *color_array,
                 int *max_array, int *min_array, int color, int num_nodes,
                 int num_edges, VertexRange range, int *local_stop)
{
    for (int tid = range.begin; tid < range.end; ++tid) {
        if (color_array[tid] == -1) {
            ...
            for (int edge = start; edge < end; ++edge) {
                ...
                if (color_array[neighbor] == -1 && start != end - 1) {
                    *local_stop = 1;
                    if (node_value[neighbor] > maximum) {
                        maximum = node_value[neighbor];
                    }
                    if (node_value[neighbor] < minimum) {
                        minimum = node_value[neighbor];
                    }
                }
            }
            max_array[tid] = maximum;
            min_array[tid] = minimum;
        }
    }
}
...
cpu_color2_range(int *node_value, int *color_array, int *max_array,
                 int *min_array, int color, VertexRange range)
{
    for (int tid = range.begin; tid < range.end; ++tid) {
        if (color_array[tid] == -1) {
            ...
            if (node_value[tid] <= min_array[tid]) {
...
int main(int argc, char **argv)
{
    ...
    if (options.debug_log) {
        fprintf(stdout,
                "color_maxmin mode: cpu_workers=%d active=%zu gpu=%s "
    ...
    if (use_gpu && gpu_end > 0) {
        ...
        hipLaunchKernelGGL(ini, dim3(num_blocks), dim3(threads), 0, 0,
                           max_array, min_array, 0, gpu_end, num_nodes);
    }
    std::vector<std::thread> init_threads;
    for (VertexRange range : cpu_ranges) {
        init_threads.emplace_back(cpu_ini_range, max_array, min_array, range);
    }
    if (use_gpu) {
        ...
        *stop_d = 0;
        ...
        for (size_t i = 0; i < cpu_ranges.size(); ++i) {
            cpu_threads.emplace_back(cpu_color1_range, csr, node_value, color,
                                     max_array, min_array, graph_color,
                                     num_nodes, num_edges, cpu_ranges[i],
                                     &cpu_stops[i]);
        }

        if (use_gpu && gpu_end > 0) {
            int num_blocks = (gpu_end + block_size - 1) / block_size;
            hipLaunchKernelGGL(color1, dim3(num_blocks), dim3(threads), 0, 0,
                               csr->row_array, csr->col_array, node_value,
                               color, stop_d, max_array, min_array,
                               graph_color, 0, gpu_end, num_nodes, num_edges);
        }
        if (use_gpu) {
            hipDeviceSynchronize();
        }
        ...
        for (VertexRange range : cpu_ranges) {
            cpu_threads.emplace_back(cpu_color2_range, node_value, color,
                                     max_array, min_array, graph_color, range);
        }
        if (use_gpu && gpu_end > 0) {
            int num_blocks = (gpu_end + block_size - 1) / block_size;
            hipLaunchKernelGGL(color2, dim3(num_blocks), dim3(threads), 0, 0,
                               node_value, color, max_array, min_array,
                               graph_color, num_nodes, num_edges, 0, gpu_end);
        }
        if (use_gpu) {
            hipDeviceSynchronize();
        }
