...
int main(int argc, char **argv)
{
    ...
    if (options.cpu_only) {
        fprintf(stderr, "--cpu-only is not supported in color_max\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (run_gpu) {
        ...
        CHECK(hipMemcpy(max_d, color, num_nodes * sizeof(int),
                        hipMemcpyHostToDevice));
        ...
        while (stop) {
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(color1), dim3(grid),
                               dim3(threads), 0, 0, row_d, col_d, node_value_d,
                               color_d, stop_d, max_d, graph_color, 0,
                               num_nodes, num_nodes, num_edges);

            hipLaunchKernelGGL(HIP_KERNEL_NAME(color2), dim3(grid),
                               dim3(threads), 0, 0, node_value_d, color_d,
                               max_d, graph_color, 0, num_nodes, num_nodes,
                               num_edges);
