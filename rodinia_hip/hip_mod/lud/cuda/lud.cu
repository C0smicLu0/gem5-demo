...
create_matrix(float **mp, int size){
  ...
  hipMallocManaged((void**)&m, sizeof(float) * size * size, hipMemAttachGlobal);
  if ( m == NULL) {
      return RET_FAILURE;
  }
...
static func_ret_t
create_matrix_from_file_um(float **mp, const char* filename, int *size_p)
{
  int i, j, size;
  float *m;
  FILE *fp = NULL;
...
main ( int argc, char *argv[] )
{
  ...
  if (input_file) {
    ...
    ret = create_matrix_from_file_um(&m, input_file, &matrix_dim);
    if (ret != RET_SUCCESS) {
      m = NULL;
