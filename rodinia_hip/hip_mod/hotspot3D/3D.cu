...
int main(int argc, char** argv)
{
    ...
    // 原来：power/temp 先在 host 分配，再 hipMalloc device + hipMemcpy(H2D)
    // 现在：powerIn/tempIn/tempOut 直接用 managed 分配，一份指针贯通 CPU/GPU
    powerIn = (float*)checked_hip_malloc_managed(size * sizeof(float));
    tempCopy = (float*)malloc(size * sizeof(float));
    tempIn = (float*)checked_hip_malloc_managed(size * sizeof(float));
    tempOut = (float*)checked_hip_malloc_managed(size * sizeof(float));
