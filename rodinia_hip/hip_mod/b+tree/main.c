...
checked_hip_malloc_managed(size_t size)
{
	...
	// 原来常见写法：CPU malloc / hipMalloc + hipMemcpy(H2D/D2H) 维护两份数据
	// 现在：只分配一份 managed 指针，CPU/GPU 共享同一虚拟地址空间（由运行时迁移/映射）
					...
					bool verbose)
{
	...
	// 这里把整棵 B+Tree 打包成一段连续内存 mem，供 GPU kernel 直接访问。
	// 原来：mem 可能是普通 malloc 的 host 内存 + 额外 hipMalloc 的 device 内存，再 hipMemcpy 复制
	// 现在：mem 用 hipMallocManaged 分配，krecords/knodes 都在同一段 managed 内存里，
	//         wrapper 里不需要 hipMemcpy(H2D)，kernel 也能直接读写。
	mem = (char *)checked_hip_malloc_managed(malloc_size);
	freeptr = (long)mem;
	  ...
	  commandPointer++;
		switch (instruction) {
			...
			case 'k':
			{
				...
				long *currKnode;
				// 原来：currKnode/offset/keys/ans 往往是 host malloc + device hipMalloc，并在 wrapper 里 hipMemcpy 来回搬运
				// 现在：统一内存 managed 分配，CPU 初始化后 kernel 直接访问；kernel 后 CPU 直接读取结果（配合 hipDeviceSynchronize）
				currKnode = (long *)checked_hip_malloc_managed(count*sizeof(long));
				// INPUT: offset CPU initialization
				memset(currKnode, 0, count*sizeof(long));

				// INPUT: offset CPU allocation
				long *offset;
				offset = (long *)checked_hip_malloc_managed(count*sizeof(long));
				// INPUT: offset CPU initialization
				memset(offset, 0, count*sizeof(long));

				// INPUT: keys CPU allocation
				int *keys;
				keys = (int *)checked_hip_malloc_managed(count*sizeof(int));
				...
				record *ans = (record *)checked_hip_malloc_managed(sizeof(record)*count);
				...
				// free memory
				// managed 内存统一用 hipFree 释放（不再区分 host free vs device hipFree）
				hipFree(currKnode);
				hipFree(offset);
				hipFree(keys);
				hipFree(ans);
			...
			case 'j':
			{
				...
				// INPUT: currKnode CPU allocation
				long *currKnode;
				// 原来：range query 相关数组也会走 host/device 两份内存 + hipMemcpy
				// 现在：全部用 hipMallocManaged，一份指针贯通 CPU 初始化、GPU kernel、CPU 读回。
				currKnode = (long *)checked_hip_malloc_managed(count*sizeof(long));
				// INPUT: offset CPU initialization
				memset (currKnode, 0, count*sizeof(long));
				...
				long *lastKnode;
				lastKnode = (long *)checked_hip_malloc_managed(count*sizeof(long));
				// INPUT: offset CPU initialization
				memset (lastKnode, 0, count*sizeof(long));

				// INPUT: offset_2 CPU allocation
				long *offset_2;
				offset_2 = (long *)checked_hip_malloc_managed(count*sizeof(long));
				// INPUT: offset CPU initialization
				memset (offset_2, 0, count*sizeof(long));

				// INPUT: start, end CPU allocation
				int *start;
				start = (int *)checked_hip_malloc_managed(count*sizeof(int));
				int *end;
				end = (int *)checked_hip_malloc_managed(count*sizeof(int));
				...
				int *recstart;
				recstart = (int *)checked_hip_malloc_managed(count*sizeof(int));
				int *reclength;
				reclength = (int *)checked_hip_malloc_managed(count*sizeof(int));
				// OUTPUT: ans CPU initialization
				for(i = 0; i < count; i++){
					recstart[i] = 0;
					reclength[i] = 0;
