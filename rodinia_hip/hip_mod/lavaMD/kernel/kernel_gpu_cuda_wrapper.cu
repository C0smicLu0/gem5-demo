						...
						FOUR_VECTOR* fv_cpu)
{
	...
	// 原来：这里会 hipMalloc device buffer，并 hipMemcpy(H2D) 把 box/rv/qv/fv 拷到 device
	// 现在：box_cpu/rv_cpu/qv_cpu/fv_cpu 都是 managed，GPU 直接使用同一指针
	d_box_gpu = box_cpu;
	d_rv_gpu = rv_cpu;
	d_qv_gpu = qv_cpu;
	d_fv_gpu = fv_cpu;
	...
	// 原来：通过 D2H memcpy 取回结果（隐式同步）
	// 现在：统一内存下显式同步确保 CPU 可读
